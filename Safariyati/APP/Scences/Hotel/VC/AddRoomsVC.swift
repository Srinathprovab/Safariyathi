...
class AddRoomsVC: UIViewController, UITableViewDelegate, UITableViewDataSource, RoomsCountTVCellDelegate, ButtonTVCellDelegate {
    
    @IBOutlet weak var topdragView: UILabel!
    ...
    static var newInstance: AddRoomsVC? {
        let storyboard = UIStoryboard(name: Storyboard.Hotel.name,
                                      bundle: nil)
        let vc = storyboard.instantiateViewController(withIdentifier: self.className()) as? AddRoomsVC
        return vc
    }
    ...
    func setupUI() {
        topdragView.layer.cornerRadius = 2
        self.view.backgroundColor = .black.withAlphaComponent(0.4)
        setuplabels(lbl: titlelbl, text: "Select Guest & Room", textcolor: .ApplabelColor, font: .InterMedium(size: 20), align: .left)
